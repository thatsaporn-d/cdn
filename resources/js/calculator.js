...
function calSuggestedPrice(isDI, mbps, price, Hprice, bw_down, bw_up){
	var bd = (bw_down ? bw_down : 0);
	var bp = (bw_up ? bw_up : 0);
	var bw = bd+bp;
	var total = 0;
	var p = (price ? price : 0);
	var Hp = (Hprice ? Hprice : 0);
	if(isDI){
		if(bw <=  mbps) total = bw * p;
		else total = bw * Hp;
	}else{
		total = p;
	}
...
function calMinimumPrice(isDI, ldiscount, hdiscount, mbps, price, Hprice, bw_down, bw_up){
	var bd = (bw_down ? bw_down : 0);
	var bp = (bw_up ? bw_up : 0);
	var bw = bd+bp;
	var total = 0;
	var p = (price ? price : 0);
	var Hp = (Hprice ? Hprice : 0);
	var ldc = (ldiscount ? ldiscount : 0);
	var hdc = (hdiscount ? hdiscount : 0);
	if(isDI){
		if(bw <=  mbps) {
			total = bw * p;
			if (ldc){
				var disc = (total * ldc) / 100;
				total = total - disc;
			}
		}else {
			total = bw * Hp;
			if (hdc){
				var disc = (total * hdc) / 100;
				total = total - disc;
			}
		}
