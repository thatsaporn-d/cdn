...
function getComp(id, cells, panel) {
   ...
   switch (cells.fdType) {
                        ...
                        if (ff.getValue() == null || (ff.getValue() + '').length == 0) {
                           switch (ff.tfType) {
                              case 1:
                                 ...
                                 break;
                              case 2:
                                 ff.setValue(defaultValue != null ? defaultValue : '');
                                 break;
                              case 6:
                                 ff.setValue(defaultValue != null ? defaultValue : 0);
                                 break;
      ...
      case Senate.fd.CurrencyField3: {
         ...
         var n = new Ext.form.NumberField3({
            id: id,
            tfType: cells.tfType,
            name: cells.tfName,
            fieldLabel: label,
         ...
         function nBlur() {
            if (!hidden) {
               ...
               var v = n.getValue();
               if (!isNaN(v)) {
                  t.setValue(Ext.util.Format.number(v, '0,000.000'));
               }
            }
         }
      ...
      case Senate.fd.FileField: {
         ...
         var delFile = new Ext.Button({
            ...
            handler: function() {
               ...
               // Begin - Apichart.S:20190301 - Fix validate Attach File when on form load set allowBlank = false
               try {
                  if (get_field(hid.id.split('-').pop(), true).allowBlank != undefined) {
                     var val = get_field(hid.id.split('-').pop(), true).value;
                     if (!get_field(hid.id.split('-').pop(), true).allowBlank) {
                        blankField = 'Attach File';
                        if (!val) {
                           if (!blankField) blankField = 'Attach File';
                        } else {
                           blankField = undefined;
                        }
                        showError('กรุณากรอกข้อมูล "' + blankField + '"', 'ข้อความ');
   ...
   for (var i = 0; i < data.length; i++) {
      ...
      if (cells.fdType == Senate.fd.Tab) {
         ...
         if (cells.tfType == Senate.tf.Boolean) {
            ...
            column['renderer'] = currency3Column;
            column['align'] = 'right';
                  ...
                  var jsonBtn = {
                     ...
                     handler: function(btn) {
                        ...
                        if (valid) {
                           ...
                           var win = new Senate.FormWindow();
                           win.showMultiFilter(panel, btn.gfmID, btn.refGrid, btn.btnIndex, scriptText, btn.fmWidth != undefined ? btn.fmWidth : btn.refGrid.fmWidth, btn.fmHeight != undefined ? btn.fmHeight : btn.refGrid.fmHeight);
                        }
                     }
                  };
                  ...
                  if (gridPanel[g].gfmArr.length > 0) {
                     ...
                     for (var j = 0; j < arr.length; j++) {
                        ...
                        jsonBtn.btnIndex = j;
								...
								jsonBtnMuF.id = 'multi-' + gridPanel[g].id + '-' + j;
                        jsonBtnMuF.gfmID = arr[j].gfmID;
                        jsonBtnMuF.text = arr[j].gfmLbl;
                        jsonBtnMuF.btnIndex = j;
								jsonBtnMuF.fmWidth = Number(arr[j].gfmWD);
								jsonBtnMuF.fmHeight = Number(arr[j].gfmHH);	

                        var tbar = gridPanel[g].getTopToolbar();
                        if (!tbar.tbField)
									if(arr[j].gfmMuF != null && arr[j].gfmMuF.toString() == 'MultiFilter') tbar.add(new Ext.Button(jsonBtnMuF));
                           else tbar.add(new Ext.Button(jsonBtn));
                        else {
									if(arr[j].gfmMuF != null && arr[j].gfmMuF.toString() == 'MultiFilter') tbar.insert(0, new Ext.Button(jsonBtnMuF));
                           else tbar.insert(0, new Ext.Button(jsonBtn));
...
Ext.extend(Senate.FormWindow, Ext.Window, {
   ...
   showMulti: function(refPanel, fmID, refGrid, btnIndex, scriptText, fmWidth, fmHeight) {
		var bd = Ext.getBody();
		var wd = Math.max(0, (bd.getWidth() - 80));
		var hh = Math.max(0, (bd.getHeight() - 80));
	...
	showMultiFilter: function(refPanel, fmID, refGrid, btnIndex, scriptText, fmWidth, fmHeight) {
		var bd = Ext.getBody();
		var wd = Math.max(0, (bd.getWidth() - 80));
		var hh = Math.max(0, (bd.getHeight() - 80)); 
               ...
               if (form.details) {
                  ...
                  if (grid && grid.fdType == Senate.fd.GridMulti) {
                     ...
                     for (var i = 0; i < refStore.getCount(); i++) {
						...
						var grid1 = form.details[1];
                  if (grid1 && grid1.fdType == Senate.fd.GridMulti) {
                     grid1.selRec = new Array();
                     for (var i = 0; i < refStore.getCount(); i++) {
                        var r = refStore.getAt(i);
                        var keys = r.get(grid1.isPK[0]) + ':' + r.get(grid1.isPK[1]);
                        grid1.selRec[keys] = r;
                        refGrid.tempRec[keys] = r;
                     }
                  }
            ...
            for (r in rec) {
               if (typeof rec[r] != 'function') {
                  ...
                  if (temp[r]) {
                     ...
                     if (form.fmID != 4002065) refGrid.store.add(rec[r]);
                  }
               }
            }
   ...
   showFilter: function(fmID, panel, params, scriptText) {
      ...
      this.getTopToolbar().add(new Ext.Button({
         text: 'Filter',
         handler: function() {
            var panel = Ext.getCmp('filter-form');
            var filterStr = '';
            for (var i = 0; i < panel.dataFields.length; i++) {
               var field = panel.dataFields;
               if (panel.findById(field[i]) != null) {
      ...
      this.addButton(new Ext.Button({
         text: 'Select',
         handler: function() {
            var grid = Ext.getCmp('filter-form').details[0];
            if (grid) {
               var frm = panel.getForm();
               var rec = grid.selRec;
               for (r in rec) {
   ...
   showForm: function(refPanel, fmID, grid, width, height, rowIndex, customId) {
      ...
      if (rowIndex != undefined) {
         this.addButton(new Ext.Button({
            ...
            handler: function() {
               if (!win.validateForm(form.getForm())) {
                  return;
               }
                  ...
                  for (var i = 0; i < f.details.length; i++) {
                     ...
                     for (var j = 0; j < refPanel.details.length; j++) {
                        ...
                        if (detGrid.tbID == subGrid.tbID) {
                           ...
                           var Rec = detStore.recordType;
                           var subStore = subGrid.getStore();
                           var records = subStore.getRange();
                           for (var k = 0; k < records.length; k++) {
                              detStore.add(new Rec(records[k].data));
                           }
